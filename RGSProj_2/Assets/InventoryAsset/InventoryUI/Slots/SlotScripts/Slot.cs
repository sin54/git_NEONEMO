...
namespace InventorySystem
{
    ...
    public class Slot : MonoBehaviour, IPointerClickHandler
    {
        ...
        private ActionSlot sIM;
        [SerializeField]private bool isActionSlot;
        private WeaponSlot wIM;
        ...
        private void Start()
        {
            ...
            if (inventoryUIManager.GetInventoryName() == "Active")
            {
                if (GetComponent<ActionSlot>() == null)
                {
                    gameObject.AddComponent<ActionSlot>();
                }
            }
            else if (inventoryUIManager.GetInventoryName() == "Weapon")
            {
                if (GetComponent<WeaponSlot>()==null)
                {
                    gameObject.AddComponent<WeaponSlot>();
                }
            }

            sIM = GetComponent<ActionSlot>();
            wIM=GetComponent<WeaponSlot>();
        ...
        public void UpdateSlot()
        {
            ...
            // OnEnter
            if ((prevItem == null || prevItem.GetIsNull()) && item != null && !item.GetIsNull())
            {
                if (sIM != null) sIM.OnEnter(item);
            }
            // OnExit
            else if (prevItem != null && !prevItem.GetIsNull() && (item == null || item.GetIsNull()))
            {
                if (sIM != null) sIM.OnExit(prevItem);
            }
            // 다른 아이템으로 변경 → OnExit + OnEnter
            else if (prevItem != null && !prevItem.GetIsNull() && item != null && !item.GetIsNull() && prevItem != item)
            {
                if (sIM != null)
                {
                    sIM.OnExit(prevItem);
            ...
            if (item != null)
            {
                ...
                else
                {
                    ...
                    SlotItemHolder.GetComponent<RectTransform>().anchoredPosition = Vector2.zero;
                }
            }
            else
            {
        ...
        private void Update()
        {
            // 아이템이 비어있지 않고 sIM이 있으면 OnActivate 매 프레임 호출
            if (item != null && !item.GetIsNull() && sIM != null)
            {
                sIM.OnActivate(item);
            }
        }
