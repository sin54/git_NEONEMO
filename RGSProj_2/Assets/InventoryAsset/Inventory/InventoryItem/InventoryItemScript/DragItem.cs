...
namespace InventorySystem
{
    ...
    internal class DragItem : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler
    {
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            foreach (RaycastResult result in results)
            {
                if (result.gameObject.CompareTag("Slot"))
                {
                    ...
                    if (slot.GetItem().GetIsNull() && slot.GetInventoryUI().GetInventory().CheckAcceptance(item.GetIsWeapon()))
                    {
        ...
        private void HandleSlot(RaycastResult result)
        {
            ...
            bool fromAccepted = targetSlot.GetInventoryUI().GetInventory().CheckAcceptance(item.GetIsWeapon());
            bool toAccepted = CurrentSlot.GetInventoryUI().GetInventory().CheckAcceptance(targetItem.GetIsWeapon());

            bool canSwap = !item.GetIsNull() && !targetItem.GetIsNull() && fromAccepted && toAccepted;

            if (canSwap)
            {
                string fromInv = CurrentSlot.GetInventoryUI().GetInventoryName();
                string toInv = targetSlot.GetInventoryUI().GetInventoryName();

                int fromPos = CurrentSlot.GetPosition();
                int toPos = targetSlot.GetPosition();

                // Remove both items first without UI updates (invoke=false)
                InventoryController.instance.GetInventory(fromInv).RemoveItemHelper(item, fromPos, false);
                InventoryController.instance.GetInventory(toInv).RemoveItemHelper(targetItem, toPos, true);

                // Swap positions
                InventoryController.instance.AddItemPos(fromInv, targetItem, fromPos);
                InventoryController.instance.AddItemPos(toInv, item, toPos);
            ...
            {
                // 기존 로직
                bool slotNull = targetItem.GetIsNull();
                bool itemAccepted = fromAccepted;

                if (slotNull && itemAccepted)
                {
                    InventoryController.instance.AddItemPos(
                        targetSlot.GetInventoryUI().GetInventoryName(), item, targetSlot.GetPosition());
                    targetSlot.GetInventoryUI().UnHighlight(result.gameObject);
                    if (prevslot != null) prevslot.GetComponent<Slot>().GetInventoryUI().ResetHighlight();
                    Destroy(gameObject);
                }
                ...
                {
                    if (itemAccepted)
                        HandleInvalidPlacementOverInv(targetItem);
                    else
