...
namespace InventorySystem
{
    ...
    public class Inventory
    {
        ...
        private bool canWeapon;
        private bool canPassive;
        ...
        private Dictionary<ItemTag, int> tagCounts;
        ...
        /// </summary>
        public void Init(bool isW,bool isP)
        {
            exceptions = new HashSet<string>();
            itemPositions = new Dictionary<string, List<int>>();
            tagCounts = new Dictionary<ItemTag, int>();
            canWeapon = isW;
            canPassive = isP;
        }
        ...
        public void AddItemPos(int index, InventoryItem item)
        {
            ...
            if (!CheckAcceptance(item.GetIsWeapon()))
            {
        ...
        /// </summary>
        public bool CheckAcceptance(bool isWeapon)
        {
            if (isWeapon)
            {
                if (canWeapon) { return true; }
                else { return false; }
            }
            else
            {
                if (canPassive) { return true; }
                else { return false; }
            }
        }
        ...
        public void SetTagDict(Dictionary<ItemTag, int> tagCounts)
        {
            this.tagCounts = tagCounts;
        }
        public IEnumerable<InventoryItem> GetActiveItems()
        {
        ...
        public void AddTagsFromItem(InventoryItem item)
        {
            foreach (ItemTag tag in Enum.GetValues(typeof(ItemTag)))
            {
                if (item.GetTags().HasFlag(tag))
                {
        ...
        public void RemoveTagsFromItem(InventoryItem item)
        {
            foreach (ItemTag tag in Enum.GetValues(typeof(ItemTag)))
            {
                if (item.GetTags().HasFlag(tag))
                {
                    if (tagCounts.ContainsKey(tag))
                    {
                        ...
                        if (tagCounts[tag] <= 0)
                            tagCounts.Remove(tag); // 0 이하일 땐 삭제
                    }
                }
            }
        }
        public int GetTagCount(ItemTag tag)
        {
            return tagCounts.TryGetValue(tag, out int count) ? count : 0;
        }
